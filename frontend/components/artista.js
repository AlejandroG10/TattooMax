...
const Artista = (props) =>{
    ...
    async function fetchHorario() {
        const data = await fetch('http://localhost:5000/horarios')
        return data;
    }

    async function fetchOpiniones(){
        const op = await fetch('http://localhost:5000/opiniones').then(res => res.json()).then(data => setOpiniones(data))
    }
    ...
    const anadirCom = () => {
        ...
        fetch('http://localhost:5000/opiniones/add', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(datos)})
