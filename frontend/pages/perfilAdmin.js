...
export default function PerfilAdmin() {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/consultas').then(res => res.json()).then(data => {setConsultas(data)})
        fetch('http://localhost:5000/citas').then(res => res.json()).then(data => {setCitas(data)})
    ...
    //     fetch('http://localhost:5000/users/'+user, {method: 'DELETE', headers: {"Content-Type": "application/json"}})
    // }
    ...
    const anadeCita = () => {
        ...
        fetch('http://localhost:5000/citas/add', {method: 'POST', headers: {"Content-Type": "application/json"}, body: JSON.stringify(datos)})
