...
export default function Perfil() {
  ...
  async function fetchUser(){
    const data = await fetch('http://localhost:5000/users').then(res => res.json()).then(data => {setUser(data.find(usuario => usuario._id == userId))})
    // console.log(user)
    }
  ...
  const actualizaDatos = () => {
    ...
    fetch('http://localhost:5000/users/'+localStorage.getItem('token'), {method: 'PUT', headers: {"Content-Type": "application/json"}, body: JSON.stringify(datos)})
